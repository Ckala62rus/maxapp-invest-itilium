...
- Production home UI no longer shows prototype state samples, summary counter cards, or MAX bridge debug data. Debug panels and the registration navigation entry are gated by `import.meta.env.DEV` or `VITE_DEBUG_UI=true`; primary navigation is now a burger menu instead of the old tab strip.
