...
- `GET /api/v1/tickets/{number}/comments` calls ITILIUM `GET /list_comment?id=&sc_number=` and maps `sender`/`comment`/`date_sending` for the ticket card timeline with client-side pagination (5 per page). Back from details uses `detailsOrigin` → myTickets / responsible / search.
...
- Dev/prod configs set outbound ITILIUM timeout to `50s` because ticket creation and marketing requests can wait on slow 1C processing. Production HTTP `write_timeout` `120s`, nginx `proxy_read_timeout`/`proxy_send_timeout` `180s`, axios create `180s`; UI `withBusyModal` ждёт до ответа. Убрано ложное «проверьте Мои заявки» при ERR_NETWORK/413 — только реальный axios timeout.
...
- ITILIUM policy (customer): **20 MiB** per attachment. Live 413 on `1c.tdbars.ru/itilium_test` with ~8 MiB JPG + HTML `nginx` ⇒ **reverse proxy before 1C is stricter than policy** — admins must set `client_max_body_size 20m` on that nginx. Our edge nginx: 40m; backend accepts up to 15 MiB/file, 32 MiB multipart. App compresses only photos **> 4 MiB** (target ≤ 19 MiB), not small camera JPEGs. If 413 persists with file &lt; 20 MiB, escalate to 1C infra, not MAX app nginx.
