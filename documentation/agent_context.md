...
- For production launch, follow `documentation/production_deployment.md`: deploy under `/opt/docker-shared/maxapp-invest-itilium`, place administrator-provided cert/key under `/etc/letsencrypt/live/<domain>/`, fill `.env`, run `sudo docker compose up --build -d`, then verify `/healthz`, `/readyz`, MAX auth and one live ITILIUM ticket flow.
