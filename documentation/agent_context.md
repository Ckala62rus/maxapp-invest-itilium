...
- Marketing request creation sends `ExecutionDate` to 1C as `YYYY-MM-DD` (same as HTML date input, e.g. `2026-06-04`); legacy `DD.MM.YYYY` input is normalized to ISO. Retries (urlencoded body, query variants) remain for no-file requests when 1C rejects transport; JSON string business errors from 1C are surfaced directly.
