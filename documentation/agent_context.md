...
- Production deployment now uses nginx HTTPS with host-mounted certbot webroot (`/var/www/certbot`) and Let's Encrypt storage (`/etc/letsencrypt`), plus closed internal backend networking. Postgres is intentionally removed from the production compose because runtime code currently uses `MemoryUserRepository`, Redis cache and ITILIUM as the source of truth; migrations remain dev/tooling scaffolding only.
...
- For production launch, follow `documentation/production_deployment.md`: issue the first cert with certbot webroot, fill `.env`, run `docker compose up --build -d`, then verify `/healthz`, `/readyz`, MAX auth and one live ITILIUM ticket flow.
