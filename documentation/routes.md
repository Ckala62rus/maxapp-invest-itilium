...
- Middleware: `RequireIdentity`
- Flow: создаёт маркетинговую заявку через `POST /create_sc_Marketing` multipart (`id`, `Services`, временно пустые `Subdivision`/`ExecutionDate`, `files` и service-specific поля).
